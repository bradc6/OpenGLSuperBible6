...
int main(int argc, char* argv[])
{
    ...
    while(true)
    {
        ...
        for(int currentBuildingCube = 0; currentBuildingCube < NUMBER_OF_CUBES; currentBuildingCube++)
        {
            GLfloat time =  currentBuildingCube + ((((GLfloat)SDL_GetTicks() * 750) / (GLfloat)CLOCKS_PER_SEC));
            GLfloat slowTime =  currentBuildingCube + (time / 3.0f);
#pragma message ("Issue related to the modelViewMatrix")
            glm::mat4 modelViewMatrix = glm::translate(glm::vec3(0.0f, 0.0f, -20.0f)) *
