...
int main(int argc, char* argv[]) {
	...
	SDL_Window *mainWindow = SDL_CreateWindow("Draw Triangle onto the screen",
	...
	std::string vertexShaderSource = LoadFileToString(
            QUOTE(SOURCEDIR/Source/Tutorial5/Shaders/Main.vs.glsl));
	...
	glShaderSource(vertexShader, 1, (const GLchar **) &sourceVertexShaderBegin,
	NULL);
	...
	std::string fragmentShaderSource = LoadFileToString(
            QUOTE(SOURCEDIR/Source/Tutorial5/Shaders/Main.fs.glsl));
	const char *sourceFragmentShaderBegin = fragmentShaderSource.c_str();
	GLuint fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(fragmentShader, 1,
			(const GLchar **) &sourceFragmentShaderBegin, NULL);
	...
	while (true) {
		...
		GLfloat time = ((GLfloat) SDL_GetTicks() * 750)
				/ (GLfloat) CLOCKS_PER_SEC;

		GLfloat currentTriangleOffset[] = { (float) sin(time) * 0.5f,
				(float) cos(time) * 0.6f, 0.0f, 0.0f };
